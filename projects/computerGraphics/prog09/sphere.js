    ...
    constructor(gl, nLon, nLat, color) {
        ...
        for (var i = 0; i <= nLat + 1; i++) {
            ...
            for (var j = 0; j <= nLon; j++) {
                ...
                // EXERCISE 1
                // This is wrong.
                var u = 1.0 - lon / (2.0 * Math.PI);   // flip horizontally so the face looks left
                var v = 1.0 - lat / Math.PI;           // make v=0 at bottom, v=1 at top (upright)
                var tcoord = new PV(u, v, 0.0, 1.0);   // store as a 4-component PV (u, v, 0, 1)
